...
import type { LifeGoal, LifeGoalDomain, LifeGoalStatus, FinancialGoal, Asset, ExchangeRate } from '../lib/types';
...
function financialGoalProgress(fg: FinancialGoal, assets: Asset[], rates: ExchangeRate[]): number {
...
export default function Goals() {
  ...
  async function load() {
    setLoading(true);
    const [{ data: goalData }, { data: fgData }, { data: assetData }, { data: rateData }] = await Promise.all([
      ...
      <div className="space-y-4 mb-4">
        {filtered.map((goal) => {
          const linkedFg = goal.linked_financial_goal_id
            ...
            : undefined;
          const progressPct = linkedFg ? financialGoalProgress(linkedFg, assets, rates) : goal.progress_pct;
            ...
            <div key={goal.id} className="border border-line rounded-sm bg-white p-5">
              ...
              {linkedFg && (
                <p className="text-xs text-ink-soft mt-1">
                  Pulled automatically from "{linkedFg.name}" in Finance → Goals
                </p>
              )}
