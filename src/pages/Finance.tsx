...
function formatMonth(monthISO: string): string {
  return new Date(monthISO + 'T00:00:00').toLocaleDateString('en-GB', {
    month: 'short',
    year: '2-digit',
  });
}
...
export default function Finance() {
  ...
  async function saveMonth(month: string, patch: { actual_net_income_dkk?: number | null; savings_dkk?: number | null }) {
    ...
    const { data } = await supabase
      ...
      .upsert(
        { user_id: user!.id, month, ...merged, id: existing?.id || undefined },
  ...
  return (
    <div>
      ...
      <p className="text-ink-soft mb-6 text-sm">
        Actual income against the plan's own illustrative bands — ranges, not guarantees.
      </p>

      {savingsRate != null && (
        <div
          className={`inline-block mb-6 px-4 py-2 rounded-sm font-mono text-sm ${
      ...
      <div className="border border-line rounded-sm bg-white p-4 mb-6" style={{ height: 280 }}>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={chartData} margin={{ top: 8, right: 12, left: -12, bottom: 0 }}>
            ...
            <Tooltip
              contentStyle={{ fontFamily: 'IBM Plex Mono', fontSize: 12, borderRadius: 2 }}
            />
            <Line
              type="stepAfter"
              dataKey="bandHigh"
                  ...
                  <tr key={m} className="border-b border-line last:border-0">
                    ...
                    <td className="px-2 py-1">
                      <input
                        ...
                        onChange={(e) =>
                          void saveMonth(m, {
                            actual_net_income_dkk: e.target.value ? Number(e.target.value) : null,
                          })
                        }
                    ...
                    <td className="px-2 py-1">
                      <input
                        ...
                        onChange={(e) =>
                          void saveMonth(m, {
                            savings_dkk: e.target.value ? Number(e.target.value) : null,
                          })
                        }
                        className="w-full px-2 py-1 bg-transparent outline-none rounded-sm focus:bg-paper-dim/40 font-mono text-xs"
                        placeholder="—"
